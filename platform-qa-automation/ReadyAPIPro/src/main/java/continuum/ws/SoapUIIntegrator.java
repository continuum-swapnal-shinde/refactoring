...
public class SoapUIIntegrator {
	...
	private static String apiFilePath = new File("").getAbsolutePath() + Utility.getMavenProperties("SoapUIFilePath");;
	...
	static SoapUIProTestCaseRunner  runner = new SoapUIProTestCaseRunner ();
		...
		private static void settingProperty(String readyAPIProjectPath) {
			...
			for (TestSuite suite : testSuites) {
				...
				if(suite.getLabel().equalsIgnoreCase("Version API Response Format Validation TestSuite")||suite.getLabel().equalsIgnoreCase("Version API Response Data Validation TestSuite") )
				{
					if(Utility.getMavenProperties("ConfigEnvironment").equalsIgnoreCase("Int")){
					suite.setPropertyValue("ServerHost","http://internal-continuum-agent-service-elb-int-1915575479.us-east-1.elb.amazonaws.com");
					}
					else if(Utility.getMavenProperties("ConfigEnvironment").equalsIgnoreCase("Prod")){
						suite.setPropertyValue("ServerHost","https://agent.service.itsupport247.net/agent/version");	
					}
					else{
					suite.setPropertyValue("ServerHost","https://staging.agent.service.itsupport247.net/agent/version");
					}
				}
				if(suite.getLabel().equalsIgnoreCase("HealthCheck API Response Format Validation TestSuite")||suite.getLabel().equalsIgnoreCase("HealthCheck API Response Data Validation TestSuite"))
				{
					if(Utility.getMavenProperties("ConfigEnvironment").equalsIgnoreCase("Int")){
						suite.setPropertyValue("ServerHost","http://internal-continuum-agent-service-elb-int-1915575479.us-east-1.elb.amazonaws.com");
					}
					else if(Utility.getMavenProperties("ConfigEnvironment").equalsIgnoreCase("Prod")){
						suite.setPropertyValue("ServerHost","https://agent.service.itsupport247.net/agent/healthCheck");
					}
					else{
						suite.setPropertyValue("ServerHost","https://staging.agent.service.itsupport247.net/agent/healthCheck");
					}
		...
		public static void runProject(String projectFile) throws Exception {
			...
			prop.put("ServerHost","http://internal-continuum-agent-service-elb-int-1915575479.us-east-1.elb.amazonaws.com");
			List<WsdlTestSuite> testSuites = project.getTestSuiteList();
			for (TestSuite suite : testSuites) {
